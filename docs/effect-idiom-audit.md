...
Applied end to end: typecheck clean and 294/294 green **with the tests switched to `DefaultWithoutDependencies`**, which proves the stub client flows through the open layer; only the two dead `FetchHttpClient` imports need deleting for `--deny-warnings`. `layer-merge-all-with-dependencies` does not fire on the current shape; nothing type-level says "these two mergeAlls must pick the same client".

**Platform-split check (this recommendation moves an import into the pure core, so it was verified against the bundling rule, not assumed):** `FetchHttpClient` lives in `@effect/platform`, and its module graph is `effect/Context` plus a fetch-based internal — no `node:*` anywhere. With the change applied, **all four** `tasks/scripts/*` device entrypoints and the library-index fixture were bundled for browser: every bundle built non-empty with zero `node:` specifiers and zero `@effect/platform-node`. The client was already in the device graph via `sync/tasker.ts`, so nothing new enters any bundle — the fulfillment just moves to the one shared module both platforms agree on. This is the boundary case, not the licence: a client that is platform-specific (e.g. `NodeHttpClient`) must never become a core `dependency`; it belongs in its platform entry point, and `DefaultWithoutDependencies` is how that entry point would override the shared choice if the platforms ever diverge.
...
**The platform-split rule (load-bearing, owner-confirmed).** The usual Effect layout — fulfill all dependencies next to the service and keep `DefaultWithoutDependencies` for tests — does not fully apply here, because dependencies in this repo *are* platform-dependent and the failure mode is silent: platform code bundled into the on-device JS. The rule: pure modules (`sync/contract.ts`, `sync/core.ts`, `profile.ts`, `tasker-api.ts`, `config.ts`, `runtime.ts`) import nothing platform-specific; each platform has a dedicated entry point (`sync/node.ts`, `sync/tasker.ts`, plus the sanctioned edges `cli.ts` and `scripts/`) where the few platform layers are provided; the split is **structural, not tree-shaken** — never rely on a bundler to drop `@effect/platform-node`. Every layer recommendation in this report was checked against it (the one that moves an import into core — `ProfileSync`'s `dependencies` — carries its bundle proof inline). Its only automated enforcement is the pair of bundle-guard tests, which is exactly why the `test/sync-tasker-bundle.test.ts:43` finding (the guard covers one of four device entrypoints) matters more than its MED label suggests: fix that one first, and the rule defends itself.
